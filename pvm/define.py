...
if not ROOT:
    ROOT = os.path.join(os.environ["HOME"],".pvm")
...
PATH_SCRIPTS_PYTHONBREW = os.path.join(PATH_SCRIPTS,"pvm")
...
# files
PATH_BIN_PYTHONBREW = os.path.join(PATH_BIN,'pvm')
...
if not PATH_HOME:
    PATH_HOME = os.path.join(os.environ["HOME"],".pvm")
...
# pvm download
PYTHONBREW_UPDATE_URL_MASTER = _get_or_default('pvm', 'master')
PYTHONBREW_UPDATE_URL_DEVELOP = _get_or_default('pvm', 'develop')
PYTHONBREW_UPDATE_URL_PYPI = _get_or_default('pvm', 'pypi')
PYTHONBREW_UPDATE_URL_CONFIG = _get_or_default('pvm', 'config')

# stable version text
PYTHONBREW_STABLE_VERSION_URL = _get_or_default('pvm', 'stable-version')
