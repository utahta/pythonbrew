...
class VenvCommand(Command):
    ...
    def run_command(self, options, args):
        ...
        if options.python:
            ...
            # check using python under pvm
            pkgname = get_using_python_pkgname()
            if not pkgname:
                logger.error('Can not use venv command before switching a python.  Try \'pvm switch <version of python>\'.')
                sys.exit(1)
    ...
    def run_command_rename(self, options, args):
        if len(args) < 3:
            logger.error("Unrecognized command line argument: ( 'pvm venv rename <source> <target>' )")
            sys.exit(1)
    ...
    def run_command_print_activate(self, options, args):
        if len(args) < 2:
            logger.error("Unrecognized command line argument: ( 'pvm venv print_activate <project>' )")
            sys.exit(1)
        
        activate = os.path.join(self._workon_home, args[1], 'bin', 'activate')
        if not os.path.exists(activate):
            logger.error('`%s` environment already does not exist. Try `pvm venv create %s`.' % (args[1], args[1]))
            sys.exit(1)
    ...
    def run_command_use(self, options, args):
        if len(args) < 2:
            logger.error("Unrecognized command line argument: ( 'pvm venv use <project>' )")
            sys.exit(1)
